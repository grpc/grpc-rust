...
fn regenerate_xds() {
    ...
    let include_dirs: Vec<PathBuf> =
        ["envoy", "xds", "protoc-gen-validate", "googleapis", "cel-spec"]
            .iter()
...
fn raw_ident(seg: &str) -> String {
    const KEYWORDS: &[&str] = &[
        ...
        "ref", "return", "static", "struct", "trait", "true", "type", "unsafe", "use",
        "where", "while", "async", "await", "dyn", "abstract", "become", "box", "do",
